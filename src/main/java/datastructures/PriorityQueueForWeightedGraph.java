...
class PriorityQueueForWeightedGraph {
    ...
    // -------------------------------------------------------------
    public PriorityQueueForWeightedGraph() // constructor
    {
    ...
    public void insert(Edge item) // insert item in sorted order
    {
        ...
        for (j = 0; j < size; j++) // find place to insert
            if (item.getWeight() >= queArray[j].getWeight())
                break;
    ...
    { // destVert value
        for (int j = 0; j < size; j++)
            if (queArray[j].getDestinationVertexIndex() == findDex)
                return j;
        return -1;
    }
}